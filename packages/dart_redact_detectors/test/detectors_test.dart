...
void main() {
  ...
  test('does not redact ordinary URL parameters', () {
    expect(redactor.sanitizeText('https://example.invalid/?page=overview&limit=20'), contains('page=overview&limit=20'));
  });
  test('does not match invalid IPv4 values', () {
    expect(redactor.scan('version 999.999.999.999').where((m) => m.type == SecretType.ipv4), isEmpty);
  });
}
